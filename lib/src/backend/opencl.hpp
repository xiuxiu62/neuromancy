...
//     clEnqueueNDRangeKernel(ctx.queue, ctx.kernels.matmul, 1, nullptr, &global_work_size, nullptr, 0, nullptr,
//     nullptr);
...
//     f32 *storage = static_cast<f32 *>(malloc(sizeof(f32) * (layer.input_size * layer.output_size +
//     layer.output_size))); f32 *weight_updates = storage; f32 *bias_updates = storage + (layer.input_size *
//     layer.output_size);

//     clEnqueueReadBuffer(ctx.queue, d_input_gradients, CL_TRUE, 0, sizeof(f32) * layer.input_size, layer.deltas, 0,
//                         nullptr, nullptr);
//     clEnqueueReadBuffer(ctx.queue, d_weight_gradients, CL_TRUE, 0, sizeof(f32) * layer.input_size *
//     layer.output_size,
...
//     layer.weights = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE, sizeof(f32) * layer.input_size *
//     layer.output_size,
//                                    nullptr, nullptr);
...
//     clEnqueueWriteBuffer(ctx.queue, layer.biases, CL_TRUE, 0, sizeof(f32) * layer.output_size, layer.biases, 0,
//     nullptr,
//                          nullptr);
// }
...
// // TODO: Load chunks of training data in, training incrementally if a region of the training data exceeds the vram
// cap
