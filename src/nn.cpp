...
//     if (storage == nullptr) {
//         warn("Failed to allocate memory for a layer");
//         exit(1);
//     }
...
//     usize total_size = sizeof(usize) + sizeof(f32); // For the layer count and learning_rate
//     for (usize i = 0; i < network.layer_count; i++) {
//         total_size += sizeof(usize) * 2; // For input and output sizes
//         total_size += sizeof(f32) * network.layers[i].input_size * network.layers[i].output_size; // For weights
//         total_size += sizeof(f32) * network.layers[i].output_size;                                // For biases
//     }
//     return total_size;
// }
...
void init(Layer &layer, usize input_size, usize output_size) {
    ...
    f32 weight_scale = std::sqrtf(2.0f / (input_size + output_size));
...
void serialize(Layer &layer, char **buffer) {
    ...
    *buffer += sizeof(usize);
    ...
    auto weight_size = sizeof(f32) * layer.input_size * layer.output_size;

    memcpy(*buffer, layer.weights, weight_size);

    *buffer += weight_size;

    auto bias_size = sizeof(f32) * layer.output_size;

    memcpy(*buffer, layer.weights, bias_size);
...
void deserialize(Layer &layer, char **buffer) {
    ...
    memcpy(&layer.input_size, *buffer, sizeof(usize));

    *buffer += sizeof(usize);

    memcpy(&layer.output_size, *buffer, sizeof(usize));

    *buffer += sizeof(usize);

    memcpy(layer.weights, *buffer, layer.input_size * layer.output_size * sizeof(f32));

    *buffer += layer.input_size * layer.output_size * sizeof(f32);

    memcpy(layer.biases, *buffer, layer.output_size * sizeof(f32));

    *buffer += layer.output_size * sizeof(f32);
}
...
void deserialize(Network &network, char *buffer) {
    ...
    memcpy(&network.learning_rate, buffer, sizeof(usize));
